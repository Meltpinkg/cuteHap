...
class cuteHap(object):
	...
	Current version: v%s
	Contact: sqcao@stu.hit.edu.cn


	"""%(VERSION)

def parseArgs(argv):
	parser = argparse.ArgumentParser(prog="cuteHap", 
		description=cuteHap.USAGE, 
	...
	# **************Parameters in genotying******************
	GroupGenotype = parser.add_argument_group('Genotying')
	GroupGenotype.add_argument('--mosaic',
		help = "Enable to detect mosaic SVs.",
		action="store_true")
	GroupGenotype.add_argument('--large_inv',
		help = "Enables detection of chromosomal inversion.",
		action="store_true")
...
def Generation_VCF_header(file, contiginfo, sample, argv):
	...
	file.write("##CommandLine=\"cuteHap %s\"\n"%(" ".join(argv)))
