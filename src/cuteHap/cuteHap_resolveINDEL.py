import numpy as np
from cuteHap.cuteHap_genotype import cal_CIPOS, overlap_cover, assign_gt, bench_phase
...
def resolution_INDEL(path, chr, svtype, read_count, threshold_gloab, max_cluster_bias,
    ...
    for i in range(len(pre_pos)):
        ...
        minimum_support_reads = int(max(sum(hp_count) / 10, 2))
        ...
        # if 1 == 1:
            ...
            if len(semi_clusters_list[i]) < 100:
                ...
                generate_bad_fuzzy(semi_clusters_list[i], 
                                    ...
                                    search_flag_list,
                                    max_cluster_bias)
    candidate_single_SV_gt = call_gt(path, chr, candidate_single_SV, search_bad_list, search_flag_list, max_cluster_bias, svtype, sigs_index)
...
def bench_phase_medium(path, chr, sigs_index, pre_pos):
...
def generate_bad_ultra(semi_cluster, chr, svtype, read_count, 
    ...
    action, min_size, remain_reads_ratio, clips, search_bad_list, search_flag_list, max_cluster_bias):
    
    def find_IQR_mean(data):
    ...
    if svtype == 'INS':
        read2seq = dict()
        for _ in semi_cluster:
            if _[0] not in read2seq:
                read2seq[_[0]] = dict()
            read2seq[_[0]][_[1]] = _[3]
    
    real_ans.sort(key=lambda x: len(x[2]), reverse=True)
    ...
    for allele in real_ans:
        ...
        if signalLen < min_size:
            continue
        support_read_set = set()
        phase_id_list = list()
        for _ in allele[2]:
            if _[0] not in support_read_set:
        ...
        if len(support_read_set) >= minimum_support_reads:
            if svtype == 'DEL':
                ...
                ideal_seq = '<INS>'
                allele_read2seq = dict()
                for _ in allele[2]:
                    if _[0] not in allele_read2seq:
                        allele_read2seq[_[0]] = ''
                    allele_read2seq[_[0]] += read2seq[_[0]][_[1]]
                if ideal_seq == '<INS>':
                    sorted_values = sorted(allele_read2seq.values(), key=lambda s: len(s))
                    sorted_lengths = [len(s) for s in sorted_values]
                    signalLen = int(find_IQR_mean(sorted_lengths))
                    sorted_idx = bisect.bisect_left(sorted_lengths, signalLen)
                    if sorted_idx < len(sorted_values):
                        ideal_seq = sorted_values[sorted_idx][0:signalLen]
            pos_list = [item[1] for item in allele[2]]
            len_list = [item[2] for item in allele[2]]
            CIPOS = cal_CIPOS(np.std(pos_list), len(pos_list))
            CILEN = cal_CIPOS(np.std(len_list), len(len_list))
...
def generate_balance(semi_del_cluster, chrom, svtype, read_count, 
    ...
    def find_IQR(data):
        ...
        return max(lower_bound, data[0]), min(upper_bound, data[-1])
    ...
    for hp_idx in [1, 2, 0]:
        ...
        *********************************************************************************
        ...
        for allele_hp in allele_sort:
            if (hp_idx > 0 and allele_hp[2][0] < minimum_support_reads) or (hp_idx == 0 and allele_hp[2][0] < minimum_support_reads):
                continue
            allele_hp[3] = set(allele_hp[3])
            breakpointStart = np.mean(allele_hp[0])
            min_len_1, max_len_1 = find_IQR(allele_hp[1]) ### TODO: remove isolated, then cal average
            tot_len_1 = len(allele_hp)
            signalLen = np.mean(allele_hp[1])
            ...
            if svtype == 'DEL':
                ...
                ideal_seq = '<INS>'
                for i in allele_hp[4]:
                    if len(i) >= int(signalLen):
                        ideal_seq = i[0:int(signalLen)]
                        break
            this_SV = [chrom, svtype, [np.min(allele_hp[0]), int(breakpointStart), np.max(allele_hp[0])], [min_len_1, signalLen, max_len_1, tot_len_1], support_read_count, str(CIPOS),str(CILEN), '.', genotype, '.', '.', '.', support_read_set, ideal_seq]
            clustered_list[hp_idx].append(this_SV)

    combine_list = combine_alleles(clustered_list, minimum_support_reads, read_tag, min_size)
    combine_list.sort(key=lambda x: sum(x[4]), reverse=True)

    for allele_hp in combine_list:
        breakpointStart = allele_hp[2][1]
        signalLen = int(allele_hp[3])
        modified_max_cluster_bias = min(max_cluster_bias, signalLen+50)
        ...
        allele_hp[3] = signalLen
    pos2sv = [dict(), dict(), dict(), dict()]; pos2allelenum = [dict(), dict()] # 0|0, 1|0, 0|1, 1|1
    for _ in combine_list:
        for i in range(4):
            pos2sv[i].setdefault(_[2], list())
        pos2allelenum[0].setdefault(_[2], 0); pos2allelenum[1].setdefault(_[2], 0)
        _hp = 0
        if _[8][0] == '1':
            pos2allelenum[0][_[2]] += 1; _hp += 1
        if _[8][2] == '1':
            pos2allelenum[1][_[2]] += 1; _hp += 2
        pos2sv[_hp][_[2]].append(_)
    for _pos in (pos2allelenum[0].keys() & pos2allelenum[1].keys()):
        if pos2allelenum[0][_pos] > 1 or pos2allelenum[1][_pos] > 1:
            if len(pos2sv[3][_pos]) > 0:
                pos2sv[3][_pos] = [pos2sv[3][_pos][0]]
                # others are mosaic?
            elif len(pos2sv[1][_pos]) > 1:
                for _ in pos2sv[1][_pos]:
                    _[8] = '.|.'
            elif len(pos2sv[2][_pos]) > 1:
                for _ in pos2sv[2][_pos]:
                    _[8] = '.|.'
    result_list = list()
    for i in range(4):
        for _pos in pos2sv[i]:
            result_list.extend(pos2sv[i][_pos])
    return result_list
...
def generate_bad_fuzzy(semi_cluster, chr, svtype, read_count, 
    ...
    if len(semi_cluster) < minimum_support_reads:
        return
    ...
    *********************************************************************************
    ...
    for allele in allele_sort:
        ...
        if allele[2][0] >= minimum_support_reads:
            ...
            if int(signalLen) < min_size:
            # if allele[1][-1] <= min_size:
                continue
            if svtype == 'DEL':
                ideal_seq = '<DEL>'
            else:
...
def call_gt(temporary_dir, chr, candidate_single_SV, search_bad_list, search_flag_list, max_cluster_bias, svtype, sigs_index):
    # reads_list = list() # [(start, end, 0, 'name'), ...]
    ...
    assign_list = assign_gt(iteration_dict, primary_num_dict, cover_dict, read_id_dict, phased_id_dict, read_to_phase_dict, candidate_single_SV, search_flag_list)
    # [[DV, DR, GT, GL, GQ, QUAL] ...]
    ...
    for i in range(len(candidate_single_SV)):
        ...
        if svtype == 'INS':
            candidate_single_SV_gt[i].append(candidate_single_SV[i][10])
...
def revise_single_allele(this_SV, max_cluster_bias, overlap_reads, hp_idx):
    ...
    if hp_idx == 1: hp_idx = 0 # s = s[:i] + '1' + s[i+1:]
...
def combine_alleles(a_phased_single_SV, read_count, remained_alleles, min_size):
    ...
    for allele_hp1 in a_phased_single_SV[1]:
        ...
        for allele_hp2_idx in range(len(a_phased_single_SV[2])):
            ...
            if min(mean_len_1, mean_len_2) / max(mean_len_1, mean_len_2) > 0.9 and max(min_len_1, min_len_2) <= min(max_len_1, max_len_2): # a hom SV and min_len1 overlap max_len2
                ...
                signalLen = (mean_len_1*allele_hp1[3][3] + mean_len_2*allele_hp2[3][3]) / (allele_hp1[3][3]+allele_hp2[3][3])
                ...
                if max(max_len_1, max_len_2) >= min_size:
                    if len(allele_hp1[13]) > len(allele_hp2[13]):
                        ideal_seq = allele_hp1[13][0:int(signalLen)]
                    else:
                        ideal_seq = allele_hp2[13][0:int(signalLen)]
                    result_list.append([allele_hp1[0], allele_hp1[1], [min(allele_hp1[2][0], allele_hp2[2][0]), int(breakpointStart), max(allele_hp1[2][2], allele_hp2[2][2])], signalLen, support_read_count, allele_hp1[5], allele_hp1[6], '.', genotype, '.', '.', '.', support_read_set, ideal_seq])
                break
        ...
        if not used_hp1_id:
            for allele_hp0_idx in range(len(a_phased_single_SV[0])):
                ...
                if min(mean_len_1, mean_len_2) / max(mean_len_1, mean_len_2) > 0.9 and max(min_len_1, min_len_2) <= min(max_len_1, max_len_2): # a hom SV
                    ...
                    signalLen = (mean_len_1*allele_hp1[3][3] + mean_len_2*allele_hp0[3][3]) / (allele_hp1[3][3]+allele_hp0[3][3])
                    ...
                    if max(max_len_1, max_len_2) >= min_size:
                        if len(allele_hp1[13]) > len(allele_hp0[13]):
                            ideal_seq = allele_hp1[13][0:int(signalLen)]
                        else:
                            ideal_seq = allele_hp0[13][0:int(signalLen)]
                        result_list.append([allele_hp1[0], allele_hp1[1], [min(allele_hp1[2][0], allele_hp0[2][0]), int(breakpointStart), max(allele_hp1[2][2], allele_hp0[2][2])], signalLen, support_read_count, allele_hp1[5], allele_hp1[6], '.', genotype, '.', '.', '.', support_read_set, ideal_seq])
                    break
        ...
        if allele_hp2_idx not in used_hp2_id:
            ...
            for allele_hp0_idx in range(len(a_phased_single_SV[0])):
                ...
                if min(mean_len_1, mean_len_2) / max(mean_len_1, mean_len_2) > 0.9 and max(min_len_1, min_len_2) <= min(max_len_1, max_len_2): # a hom SV
                    ...
                    signalLen = (mean_len_1*allele_hp2[3][3] + mean_len_2*allele_hp0[3][3]) / (allele_hp2[3][3]+allele_hp0[3][3])
                    ...
                    if max(max_len_1, max_len_2) >= min_size:
                        if len(allele_hp2[13]) > len(allele_hp0[13]):
                            ideal_seq = allele_hp2[13][0:int(signalLen)]
                        else:
                            ideal_seq = allele_hp0[13][0:int(signalLen)]
                        result_list.append([allele_hp2[0], allele_hp2[1], [min(allele_hp0[2][0], allele_hp2[2][0]), int(breakpointStart), max(allele_hp0[2][2], allele_hp2[2][2])], signalLen, support_read_count, allele_hp2[5], allele_hp2[6], '.', genotype, '.', '.', '.', support_read_set, ideal_seq])
                    break
        if allele_hp2_idx not in used_hp2_id:
            allele_hp2 = a_phased_single_SV[2][allele_hp2_idx]
